...
#include <user_functions/controllers/footbot_foraging.h>
...
void CForagingLoopFunctions::Init(TConfigurationNode &t_node) {
  try {
    ...
    m_cOutput.open(
      m_strOutput.c_str(), std::ios_base::trunc | std::ios_base::out);
    ...
    GetNodeAttribute(
      tForaging, "energy_per_walking_robot", m_unEnergyPerWalkingRobot);
...
CColor CForagingLoopFunctions::GetFloorColor(
  ...
  for (UInt32 i = 0; i < m_cFoodPos.size(); ++i) {
    if (
      (c_position_on_plane - m_cFoodPos[i]).SquareLength() <
      m_fFoodSquareRadius) {
      return CColor::BLACK;
    }
  }
...
void CForagingLoopFunctions::PreStep() {
  ...
  for (CSpace::TMapPerType::iterator it = m_cFootbots.begin();
    ...
    CFootBotForaging &cController = dynamic_cast<CFootBotForaging &>(
    ...
    CVector2 cPos;
    cPos.Set(
      cFootBot.GetEmbodiedEntity().GetOriginAnchor().Position.GetX(),
  ...
  /* Lock m_cOutputJson as other threads might use it */
  std::lock_guard<std::mutex> guard(m_cJsonMutex);
  m_cOutputJson["step"] = GetSpace().GetSimulationClock();
  m_cOutputJson["walking_fb"] = unWalkingFBs;
  m_cOutputJson["resting_fb"] = unRestingFBs;
  m_cOutputJson["collected_food"] = m_unCollectedFood;
  m_cOutputJson["energy"] = m_nEnergy;
}

/****************************************/
/****************************************/

const nlohmann::json CForagingLoopFunctions::GetStatus() {
  /* Lock m_cOutputJson as other threads might use it */
  std::lock_guard<std::mutex> guard(m_cJsonMutex);
  return m_cOutputJson;
}
